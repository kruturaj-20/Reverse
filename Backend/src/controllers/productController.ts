...
export const updateProduct = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (product && price !== undefined && price !== oldPrice) {
    ...
    for (const w of watchers) {
      ...
      if (!w.notifiedAt || w.notifiedAt < new Date(Date.now() - 24 * 60 * 60 * 1000)) {
        await sendPriceDropNotification(w.userId.toString(), id, price, w.targetPrice!);
...
export const getBudgetBundle = asyncHandler(async (req: Request, res: Response) => {
...
export const getPersonalizedFeed = asyncHandler(async (req: Request, res: Response) => {
  ...
  // get recent searches
  const recent = await SearchLog.find({ userId }).sort({ createdAt: -1 }).limit(5);
  ...
  const products = seedQuery
    ? await searchAffiliateProducts({ keywords: [seedQuery], rawQuery: seedQuery }, 20)
    : [];

  sendSuccess(res, products, 'Personalized home feed');
});

export const getBundleSuggestions = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const getBuyAdvice = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const product = await Product.findById(id);
  if (!product) throw new AppError('Product not found', 404);
  ...
  if (stats.length > 0) {
    const { minPrice, avgPrice } = stats[0] as { minPrice: number; avgPrice: number };
