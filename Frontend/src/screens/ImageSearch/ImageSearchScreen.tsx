...
export const ImageSearchScreen = () => {
  ...
  const runImageSearch = async (uri: string, type: string) => {
    ...
    try {
      ...
      setError(
        err?.message || 'Could not process image. Is the backend running?',
      );
    } finally {
  ...
  const handleUpload = () => {
    Alert.alert('Search by Image', 'Choose a source', [
          ...
          {imageUri ? (
                ...
                <View style={styles.processingOverlay}>
                  ...
                  >
                    <LinearGradient
                      ...
                      style={styles.scanRingGradient}
                    />
                  </Animated.View>
                  <Text style={styles.processingText}>✨ AI Analyzing...</Text>
                  <Text style={styles.processingSubtext}>
                    Gemini Vision is identifying the product
                  </Text>
                </View>
              )}
            </View>
            ...
            >
              ...
              <Text style={styles.uploadSub}>
                AI will find similar items across stores
              </Text>
        ...
        {results.length > 0 && !isProcessing && (
          <View style={styles.resultsSection}>
            ...
            <Text style={styles.resultsSubtitle}>
              {results.length} matches across stores
            </Text>
            <TouchableOpacity
              onPress={() => setCheaperOnly(v => !v)}
              style={styles.filterBtn}
            >
              <Text style={styles.filterBtnText}>
                {cheaperOnly ? 'Show all' : 'Cheaper alternatives'}
              </Text>
            </TouchableOpacity>
            <View style={styles.grid}>
              {(cheaperOnly
                ? results.filter(p => {
                    const avg =
                      results.reduce((sum, r) => sum + (r.price || 0), 0) /
                      results.length;
                    return (p.price || 0) < avg;
                  })
                : results
              ).map(p => (
